...
public:
    ...
    Canvas render() override { Canvas c; c.draw(text); return c; }
    std::string click(int x, int y) override { return "Label("+text+")"; }
};
...
class CheckBox: public Leaf {
    ...
    bool ticked;
public:
    CheckBox(const std::string& text, const bool ticked):text(text),ticked(ticked){}
    Canvas render() override { Canvas c; c.draw("["+std::string(ticked?"x":" ")+"] "+text); return c; }
    std::string click(int x, int y) override { return "CheckBox(" + text + ")"; }
};
// cut
